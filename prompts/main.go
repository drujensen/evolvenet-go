...
// Example of semantic network usage
func demonstrate_semantic_network() {
	fmt.Println("=== Semantic Network Demo ===")

	// Create a semantic layer with 100 word capacity, 50-dim embeddings, 3-word context
	semantic_layer := NewSemanticLayer(100, 50, 3)

	// Add some words with simple embeddings (in practice, use pre-trained embeddings)
	words := []string{"cat", "dog", "animal", "pet", "food", "ball", "play", "run"}
	embeddings := make([][]float64, len(words))

	for i, word := range words {
		// Create simple embeddings (random for demo)
		embedding := make([]float64, 50)
		for j := range embedding {
			embedding[j] = randFloat(-1.0, 1.0)
		}
		embeddings[i] = embedding
		semantic_layer.add_word(word, embedding)
	}

	// Simulate a train of thought
	context := []string{"cat"}
	fmt.Printf("Starting context: %v\n", context)

	for i := 0; i < 5; i++ {
		next_word := semantic_layer.get_next_word(context)
		if next_word != "" {
			context = append(context, next_word)
			fmt.Printf("Next word: %s, Context: %v\n", next_word, context)
		} else {
			break
		}
	}
